...
| configMap.authentication_backend.ldap.additional_groups_dn | string | `""` | An additional dn to define the scope of groups. |
| configMap.authentication_backend.ldap.additional_users_dn | string | `""` | An additional dn to define the scope to all users. |
| configMap.authentication_backend.ldap.address | string | `""` | The address for the ldap server. Format: <scheme>://<address>[:<port>]. Scheme can be ldap or ldaps in the format (port optional). |
...
| configMap.authentication_backend.ldap.user | string | `""` | The username of the admin user. |
...
| configMap.identity_providers.oidc.claims_policies | object | `{}` | The claims policies are policies which allow customizing the behavior of claims and the available claims for a particular client. |
...
| configMap.identity_providers.oidc.discovery_signed_response_alg | string | `""` | The JSON Web Algorithm used for signing the discovery metadata. |
| configMap.identity_providers.oidc.discovery_signed_response_key_id | string | `""` | The JSON Web Key ID used for signing the discovery metadata. |
...
| configMap.identity_providers.oidc.enable_jwt_access_token_stateless_introspection | bool | `false` | Enables introspecting JWT Profile Access Tokens using stateless checks. This is strongly discouraged. |
...
| configMap.identity_providers.oidc.require_pushed_authorization_requests | bool | `false` | Enables requiring all Authorization Requests to be initiated using a Pushed Authorization Request. |
...
| configMap.server.endpoints.rate_limits.session_elevation_finish.enable | bool | `true` | Enables this rate limit. |
| configMap.server.endpoints.rate_limits.session_elevation_start.buckets | list | `[]` | List of rate limit buckets. |
| configMap.server.endpoints.rate_limits.session_elevation_start.enable | bool | `true` | Enables this rate limit. |
...
| configMap.session.expiration | string | `"1 hour"` | The time before the cookie expires and session is reset. |
| configMap.session.inactivity | string | `"5 minutes"` | The inactivity time before the session is reset. |
...
| ingress.traefikCRD.middlewares.auth.enableMaxResponseBodySize | bool | `false` | Sets an appropriate value for maxResponseBodySize. |
...
| persistence.extraPersistentVolumeClaims.example.accessModes | list | `["ReadWriteOnce"]` | PersistentVolumeClaim access modes. |
...
| persistence.extraPersistentVolumeClaims.example.size | string | `"100Mi"` | PersistentVolumeClaim volume size. |
| persistence.extraPersistentVolumeClaims.example.storageClass | string | `""` | Uses the specified storageClass for the PersistentVolumeClaim. |
...
| persistence.subPathExpr | string | `""` | Uses an expression to mount a subpath of the PersistentVolumeClaim. |
